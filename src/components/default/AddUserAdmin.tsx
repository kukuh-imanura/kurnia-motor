...
const AddUserAdmin = () => {
  ...
  const saveUser = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:5000/api/auth/registerAdmin", {
        name,
        tlp,
        email,
        username,
          ...
          <div className="w-1/2">
            <form onSubmit={saveUser} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
              ...
              <div className="mb-4">
                ...
                <input type="text" id="tlp" value={tlp} onChange={(e) => setTlp(e.target.value)} placeholder="No. Hp" className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline " onWheel={(event) => event.currentTarget.blur()} />
              </div>
